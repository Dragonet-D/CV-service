...
export async function getUserGroupUserList(obj) {
  const userGroupUserList = _.cloneDeep(urls.userGroupUserList);
  const params = {
    userId: obj.userId,
    groupId: obj.groupId !== undefined ? obj.groupId : '',
    filter: obj.keyWord !== undefined ? obj.keyWord : '',
    removeMe: obj.removeMe !== undefined ? obj.removeMe : '',
    groupName: obj.groupName !== undefined ? obj.groupName : ''
  };
  // userGroupUserList.url = `${userGroupUserList.url}?userId=${userId}&groupId=${groupId}&filter=${keyWord}&removeMe=${removeMe}&groupName=${groupName}`;
  return fetch.post(userGroupUserList, params);
}

export async function getAllDeviceApi(object) {
  return fetch.post(vmsUrls.channels, {
    pageNo: object.pageNo,
    pageSize: object.pageSize,
    userGroupId: object.groupId,
    userId: object.uid,
