...
export async function getLiveInstanceListApi({
  pindex = 0,
  psize = 5,
  instanceName = '',
  appName = '',
  ...
  return fetch.get(urls.getLiveVAIstanceList);
}
...
export async function getJobInstanceListApi({
  pindex = 0,
  psize = 5,
  instanceName = '',
  appName = '',
  ...
  return fetch.get(urls.getJobVAInstancesList);
}
...
export async function getServiceInstanceListApi({
  pindex = 0,
  psize = 5,
  instanceName = '',
  appName = '',
  ...
  return fetch.get(urls.getServiceVAInstancesList);
}
...
// vap files
export async function getUserUploadFilesApi({ pindex, psize, name, mimetype }) {
  const urls = _.cloneDeep(getUrls.vap);
  const nameQuery = `&name=${name}`;
  const mimetypeQuery = `&mimetype=${mimetype}`;
  urls.getUserUploadFiles.url = `${urls.getUserUploadFiles.url}?pIndex=${pindex}&pSize=${psize}${
    name ? nameQuery : ''
...
export async function getIncidentApi(obj) {
  ...
  return fetch.get(urls.getIncidentList);
}
